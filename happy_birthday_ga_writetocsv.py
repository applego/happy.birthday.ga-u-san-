...
def get_objective_sentence():
    ...
    #objective_sentence = 'Tooth OK, ' + str(USER_NAME) + '!'
...
if __name__ == '__main__':
    ...
    #追記 20180808
    str_out = ""
    #f = open(r"D:\src\happy.birthday.ga-master\output.csv",'a')
    f = open(r"D:\src\happy.birthday.ga-master\output_"+datetime.now().strftime('%Y%m%d_%H%M%S')+".csv",'x')
    ...
    while max(evaluations) < 1:
        ...
        #追記　20180808
        data = [str(generation_id),''.join(agents[np.argmax(evaluations)]),str(max(evaluations))]
        #print(data)
        #print(agents)
        #print(evaluations)
        #print(objective_sentence)

        str_out += ",".join(data) + "\n"
        #print(str_out)
        # if(len(csv_list) == 0):
        #     csv_list.append(str(generation_id))
        #     csv_list.append(','.join(agents[np.argmax(evaluations)]))
        #     csv_list.agents(','.join(str(max(evaluations)))+'\\')
        # else:
        #     csv_list.append(str(generation_id))
        #     csv_list.append(','.join(agents[np.argmax(evaluations)]))
        #     csv_list.agents(','.join(str(max(evaluations)))+'\\')
